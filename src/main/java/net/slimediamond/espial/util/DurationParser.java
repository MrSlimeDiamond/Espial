...
public class DurationParser {
    ...
    public static long parseDurationAndSubtract(String input) {
        Pattern pattern = Pattern.compile(
                "(\\d+)([dhms])"); // Match the pattern for numbers followed by units (d, h, m, s)
        ...
        if (matcher.matches()) {
            ...
            switch (unit) {
                case "d": // Days
                    durationInMillis = value * 24 * 60 * 60 *
                            1000L; // Convert to milliseconds
                    break;
                case "h": // Hours
                    durationInMillis =
                            value * 60 * 60 * 1000L; // Convert to milliseconds
                    break;
                case "m": // Minutes
                    durationInMillis =
                            value * 60 * 1000L; // Convert to milliseconds
                    break;
                case "s": // Seconds
            ...
            return currentTimeInMillis -
                    durationInMillis; // Return the past time in milliseconds
        } else {
            throw new IllegalArgumentException(
