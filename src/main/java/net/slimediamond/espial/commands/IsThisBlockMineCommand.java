                    ...
                    Espial.getInstance()
                            ...
                            .ifPresentOrElse(user -> {
                                if (user.profile().uuid().equals(player.profile().uuid())) {
                                    context.sendMessage(Component.text("This block was placed by you!")
                                            .color(NamedTextColor.GREEN));
                                } else {
                                    // We have a user, but it's not them, so it's not their block!
                                    context.sendMessage(Component.text("This block was placed by another player.")
                                            .color(NamedTextColor.YELLOW));
                                }
                            }, () -> {
                                // No user can be found for this block. It's probably not in the
                                // database.
                                context.sendMessage(Component.text("We are not sure who placed this block.")
                                        .color(NamedTextColor.RED));
                            });
                } catch (SQLException | ExecutionException | InterruptedException e) {
                    context.sendMessage(Component.text("Unfortunately, an error occurred when looking up this block's owner. " +
                                    "If this persists, tell an admin!")
                            .color(NamedTextColor.RED));
                    throw new RuntimeException(e);
                }
            }, () -> context.sendMessage(Component.text("Could not find the block in front of you, " +
