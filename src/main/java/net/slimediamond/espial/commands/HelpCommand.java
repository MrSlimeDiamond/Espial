    ...
    public CommandResult execute(CommandContext context) {
        ...
        if (context.hasAny(CommandParameters.HELP_COMMAND)) {
            ...
            Commands.commands.get(0).subcommands().stream()
                    .filter(cmd -> cmd.aliases().contains(command)).findFirst()
                    .ifPresent(subcommand -> {
                        ...
                        subcommand.command().flags().forEach(flag -> {
                            ...
                            String shortestAlias = aliases.stream()
                            ...
                            flags.add(Component.space()
                                    ...
                                    .append(Component.text(shortestAlias)
            ...
            List<Parameter.Subcommand> sortedSubcommands =
                    command.subcommands().stream()
                            .sorted(Comparator.comparing(
                                    subcommand -> subcommand.aliases().stream()
                                            .max(Comparator.comparingInt(
                                                    String::length)).get()))
                            .toList();

            for (Parameter.Subcommand subcommand : sortedSubcommands) {
                if (subcommand.command().executionRequirements()
                        .test(context.cause())) {
                    var builder = Component.text()
                            .append(Component.text("/espial " +
                    ...
                    Optional<Component> description = subcommand.command()
